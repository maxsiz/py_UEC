...
card_trip_count_sl = {data.rstrip().split(';')[0]:0  for data in open(file1, encoding='cp1251') 
                if len(data.rstrip().split(';')[0])==9 and int(data.rstrip().split(';')[4])==0}
...
for carrier in carrier_gorod_set:
    ...
    carrier_tickets = [cur_str.rstrip().split(';')[0]+';'#номер карты
                       ...
                       +str(math.trunc(int(cur_str.rstrip().split(';')[2])
                        #+str((int(cur_str.rstrip().split(';')[2])
                        #+str(round(int(cur_str.rstrip().split(';')[2])
                                      /card_trip_count_sl[cur_str.rstrip().split(';')[0]]
                                       *150*100#/100*100
                                      )/100 #cумма по карте
                             if    cur_str.rstrip().split(';')[0] not in card_change_sl 
                                and cur_str.rstrip().split(';')[0] not in card_change_sl.values()
                             else #кейс с перевыпуском
                            math.trunc(int(cur_str.rstrip().split(';')[2])
                             #round(int(cur_str.rstrip().split(';')[2])
                                      /card_trip_count_sl[cur_str.rstrip().split(';')[0]]
                                       *150*100#100*100/2
                                      )//2/100).replace('.',',')+'\n'    
